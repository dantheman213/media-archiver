...
fn check_executable(name: &str, custom_path: &Option<String>, bin_dir: &Path) -> (bool, Option<String>) {
    // 1. Check custom path
...
#[tauri::command]
pub fn set_binary_paths(app: AppHandle, yt_dlp_path: Option<String>, ffmpeg_path: Option<String>) -> Result<(), String> {
...
async fn download_file(app: &AppHandle, url: &str, dest: &Path, component: &str) -> Result<(), String> {
    let client = reqwest::Client::builder().user_agent("media-archiver").build().map_err(|e| e.to_string())?;
...
pub async fn install_binaries(app: AppHandle) -> Result<(), String> {
    ...
    let yt_dlp_dest = bin_dir.join(if cfg!(target_os = "windows") { "yt-dlp.exe" } else { "yt-dlp" });
    ...
    let ffmpeg_dest = bin_dir.join(if cfg!(target_os = "windows") { "ffmpeg.exe" } else { "ffmpeg" });
    let ffmpeg_archive = bin_dir.join(if cfg!(target_os = "linux") { "ffmpeg.tar.xz" } else { "ffmpeg.zip" });
